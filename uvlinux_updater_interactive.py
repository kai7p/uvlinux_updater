...
'''
script name: uvlinux_updater_interactive.py
date:        23.12.2017
...
def progress(current, top, label="Progress"):
    workdone = current/top
    if not hasattr(progress, "length"):
        progress.length = 0
    if not hasattr(progress, "start_time"):
        progress.start_time = time()

    elapsed_time = time() - progress.start_time
    totalSizeInKiloBytes = current / 1024

    if elapsed_time == 0.0:
        str1="{0:s}: [{1:30s}] {2:.1f}% - {2:.1f}".format(label,'#' * int(workdone * 30), workdone*100)
    else:
        kbps = totalSizeInKiloBytes / elapsed_time
        seconds = top - current
        seconds = seconds / kbps / 1024
        if kbps > 1024:
            mbps = kbps / 1024
            str1="{0:s}: [{1:30s}] {2:.1f}% - Download speed: {3:.1f} MB/s - Remaining time (est.): {4:.0f} s ".format(label,'#' * int(workdone * 30), workdone*100, mbps, seconds)
        else:
            str1="{0:s}: [{1:30s}] {2:.1f}% - Download speed: {3:.0f} KB/s - Remaining time (est.): {4:.0f} s ".format(label,'#' * int(workdone * 30), workdone*100, kbps, seconds)
    print(('\b'*progress.length)+str1, end="", flush=True)
    progress.length=len(str1)
    if workdone == 1:
        print()
        
        totalSizeInKiloBytes = top / 1024
        kbps = totalSizeInKiloBytes / elapsed_time
        if kbps > 1024:
            mbps = kbps / 1024
            print('Total download time: %.2dh %.2dm %.2ds - Average download speed: %.1f MB/s' % (elapsed_time//3600, (elapsed_time//60)%60, elapsed_time%60, mbps))
        else:
            print('Total download time: %.2dh %.2dm %.2ds - Average download speed: %.0f KB/s' % (elapsed_time//3600, (elapsed_time//60)%60, elapsed_time%60, kbps))
...
def getcurrent_filename_and_hash(proxy_handl, ssl_cont, url):
    ...
    try:
        ...
        if config.has_option('AVV-ZIP', 'filename') and config.has_option('AVV-ZIP', 'md5') and config.has_option('AVV-ZIP', 'filepath') and config.has_option('AVV-ZIP', 'filesize'):
            returntuple = config.get('AVV-ZIP', 'filename'), config.get('AVV-ZIP', 'md5'), config.get('AVV-ZIP', 'filepath'), config.getint('AVV-ZIP', 'filesize')
            
    return returntuple

def download_archive(base_url, filename, proxy_handl, ssl_cont, filesize):
    tempdir = gettempdir()
    ...
    with open(target_file_name, 'wb') as out_file:
        with closing(ur.urlopen(url)) as fp:
            block_size = max(4096, filesize//1000)
            while True:
                ...
                if not block:
                    ...
                    size += len(block)
                    #print('{:.2f}\r done'.format(size/filesize), end='')
...
def main():
    ...
    filename, hash_md5, filepath, filesize = getcurrent_filename_and_hash(proxy_handler, ssl_context, base_url + '/' + ini_file)
    ...
    if filename == '':
        ...
        tmp_file = download_archive(base_url + filepath, filename, proxy_handler, ssl_context, filesize)
